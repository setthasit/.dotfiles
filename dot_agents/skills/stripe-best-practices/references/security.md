...
**Negative balance liability:** Liability follows `defaults.responsibilities.losses_collector` on the Accounts v2 configuration, not a legacy account type. `losses_collector: "stripe"` leaves unresolved negative balances with Stripe; `"application"` puts them on the platform, which destination charges and separate charges and transfers require. Prefer `"stripe"` unless the charge pattern forces otherwise, and read [connect](connect.md) before choosing. Never frame this as Standard vs. Express vs. Custom accounts — those legacy types are deprecated.
