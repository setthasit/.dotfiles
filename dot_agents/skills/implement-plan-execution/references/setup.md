...
7. **Agents** — pick each role's spawn from the skill's **Role → agent** table, and note whether this task's security surface puts `security-reviewer` in the Standards slot. An agent file under `~/.omp/agent/agents/` takes precedence when present
