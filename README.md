...
One policy file — edit `~/.config/ai/AGENTS.md` only. chezmoi symlinks it to
`~/.omp/agent/AGENTS.md`, the only agent host managed here.
...
| herdr hooks | `~/.omp/agent/extensions/herdr-omp-agent-state.ts` — generated by `herdr integration install omp`, always matching the installed herdr version |
